...
var require_L_Path_Transform = __commonJS({
  "node_modules/leaflet-path-transform/dist/L.Path.Transform.js"() {
    L.Path.include({ _transform: function(t) {
      if (this._renderer) {
        if (t) {
    ...
    var END = { mousedown: "mouseup", touchstart: "touchend", pointerdown: "touchend", MSPointerDown: "touchend" };
    var MOVE = { mousedown: "mousemove", touchstart: "touchmove", pointerdown: "touchmove", MSPointerDown: "touchmove" };
    function distance(t, a) {
      var i = t.x - a.x, n = t.y - a.y;
      return Math.sqrt(i * i + n * n);
    }
    L.Handler.PathDrag = L.Handler.extend({ statics: { DRAGGING_CLS: "leaflet-path-draggable" }, initialize: function(t) {
      this._path = t;
      this._matrix = null;
      ...
      L.DomEvent.on(document, MOVE[a], this._onDrag, this).on(document, END[a], this._onDragEnd, this);
      ...
      if (i) {
        this._path.fire("dragend", { distance: distance(this._dragStartPoint, a) });
      ...
      if (i._point) {
        a = l.unproject(l.project(i._latlng)._add(d));
        if (p) {
          i._latlng = a;
          i._point._add(s);
        }
      } else if (i._rings || i._parts) {
        var f = i._rings || i._parts;
        var g = i._latlngs;
        a = a || g;
        ...
        for (n = 0, r = f.length; n < r; n++) {
          ...
          for (var u = 0, c = f[n].length; u < c; u++) {
            e = g[n][u];
            a[n][u] = l.unproject(l.project(e)._add(d));
            if (p) {
              i._bounds.extend(g[n][u]);
              f[n][u]._add(s);
            }
          }
        }
    ...
    L.SVG.include(!L.Browser.vml ? {} : { _resetTransformPath: function(t) {
      ...
      var r = Math.floor(a[4]).toFixed() + ", " + Math.floor(a[5]).toFixed();
    ...
    L.Canvas.include({ _resetTransformPath: function(t) {
    ...
    L.PathTransform = {};
    L.PathTransform.pointOnLine = function(t, a, i) {
      var n = 1 + i / t.distanceTo(a);
      return new L.Point(t.x + (a.x - t.x) * n, t.y + (a.y - t.y) * n);
    };
    L.PathTransform.merge = function() {
      ...
      while (n) {
        ...
        for (a in n) {
          ...
          i = n[a];
          if (r(i) && r(e[a])) {
            e[a] = L.PathTransform.merge(e[a], i);
          } else {
            e[a] = i;
          }
        }
    ...
    L.Matrix = function(t, a, i, n, r, e) {
      this._matrix = [t, a, i, n, r, e];
    };
    L.Matrix.prototype = { transform: function(t) {
      ...
      var a = this._matrix;
      return new L.Point((t.x / a[0] - a[4]) / a[0], (t.y / a[2] - a[5]) / a[2]);
    }, clone: function() {
      var t = this._matrix;
      ...
      return this._add(i, 0, 0, n, a.x, a.y)._add(1, 0, 0, 1, -a.x, -a.y);
    }, rotate: function(t, a) {
      var i = Math.cos(t);
      var n = Math.sin(t);
      a = a || new L.Point(0, 0);
      return this._add(i, n, -n, i, a.x, a.y)._add(1, 0, 0, 1, -a.x, -a.y);
    }, flip: function() {
      this._matrix[1] *= -1;
      this._matrix[2] *= -1;
      return this;
    }, _add: function(t, a, i, n, r, e) {
      var s = [[], [], []];
      var o = this._matrix;
      var h = [[o[0], o[2], o[4]], [o[1], o[3], o[5]], [0, 0, 1]];
      var _ = [[t, i, r], [a, n, e], [0, 0, 1]], l;
      if (t && t instanceof L.Matrix) {
        o = t._matrix;
        _ = [[o[0], o[2], o[4]], [o[1], o[3], o[5]], [0, 0, 1]];
      }
      for (var d = 0; d < 3; d++) {
        for (var p = 0; p < 3; p++) {
          l = 0;
    ...
    L.matrix = function(t, a, i, n, r, e) {
      return new L.Matrix(t, a, i, n, r, e);
    };
    L.PathTransform.Handle = L.CircleMarker.extend({ options: { className: "leaflet-path-transform-handler" }, onAdd: function(t) {
    ...
    L.PathTransform.RotateHandle = L.PathTransform.Handle.extend({ options: { className: "leaflet-path-transform-handler transform-handler--rotate" }, onAdd: function(t) {
      L.CircleMarker.prototype.onAdd.call(this, t);
      if (this._path && this.options.setCursor) {
        this._path.style.cursor = "all-scroll";
      }
    } });
    L.Handler.PathTransform = L.Handler.extend({ options: { rotation: true, scaling: true, uniformScaling: true, maxZoom: 22, handlerOptions: { radius: 5, fillColor: "#ffffff", color: "#202020", fillOpacity: 1, weight: 2, opacity: 0.7, setCursor: true }, boundsOptions: { weight: 1, opacity: 1, dashArray: [3, 3], fill: false, noClip: true }, rotateHandleOptions: { weight: 1, opacity: 1, setCursor: true }, handleLength: 20, edgesCount: 4, handleClass: L.PathTransform.Handle, rotateHandleClass: L.PathTransform.RotateHandle }, initialize: function(t) {
      this._path = t;
      this._map = null;
      ...
      return this.transform(t, null, a);
    }, scale: function(t, a) {
      if (typeof t === "number") {
        t = L.point(t, t);
      }
      return this.transform(0, t, null, a);
    }, transform: function(t, a, i, n) {
      var r = this._path.getCenter();
      i = i || r;
      n = n || r;
      this._map = this._path._map;
      this._transformPoints(this._path, t, a, i, n);
      this._transformPoints(this._rect, t, a, i, n);
      this._transformPoints(this._handleLine, t, a, i, n);
      this._updateHandlers();
      return this;
    }, _update: function() {
      var t = this._matrix;
      for (var a = 0, i = this._handlers.length; a < i; a++) {
        var n = this._handlers[a];
      ...
      this._updateHandlers();
      t.dragging.enable();
      this._path.fire("transformed", { matrix: a, scale: n, rotation: i, layer: this._path });
    }, reset: function() {
      if (this._enabled) {
        if (this._rect) {
      ...
      for (var a = this._handlers.length - 1; a >= 0; a--) {
        t.removeLayer(this._handlers[a]);
      }
      this._createHandlers();
    ...
    }, _getProjectedMatrix: function(t, a, i, n) {
      var r = this._map;
      var e = r.getMaxZoom() || this.options.maxZoom;
      var s = L.matrix(1, 0, 0, 1, 0, 0);
      var o;
      t = t || this._angle || 0;
      a = a || this._scale || L.point(1, 1);
      if (!(a.x === 1 && a.y === 1)) {
        n = n || this._scaleOrigin;
        o = r.project(n, e);
        s = s._add(L.matrix(1, 0, 0, 1, o.x, o.y))._add(L.matrix(a.x, 0, 0, a.y, 0, 0))._add(L.matrix(1, 0, 0, 1, -o.x, -o.y));
      }
      if (t) {
        i = i || this._rotationOrigin;
        o = r.project(i, e);
        s = s.rotate(t, o).flip();
      }
      return s;
    }, _transformPoint: function(t, a, i, n) {
      return i.unproject(a.transform(i.project(t, n)), n);
    }, _transformPoints: function(t, a, i, n, r) {
      var e = t._map;
      var s = e.getMaxZoom() || this.options.maxZoom;
      var o, h;
      var _ = this._projectedMatrix = this._getProjectedMatrix(a, i, n, r);
      if (t._point) {
        t._latlng = this._transformPoint(t._latlng, _, e, s);
        var l = this._getBoundingPolygon();
        var d = Math.floor(Math.abs(l._bounds._southWest.lat - l._bounds._northEast.lat) / 2);
        t.setRadius(d);
        ...
        for (o = 0, h = p.length; o < h; o++) {
          for (var g = 0, u = p[o].length; g < u; g++) {
            f[o][g] = this._transformPoint(f[o][g], _, e, s);
            t._bounds.extend(f[o][g]);
          }
        }
      }
      t._reset();
      ...
      if (this.options.scaling) {
        this._handlers = [];
        for (var a = 0; a < this.options.edgesCount; a++) {
          this._handlers.push(this._createHandler(this._rect._latlngs[0][a], a * 2, a).addTo(this._handlersGroup));
        }
      }
      if (this.options.rotation) {
      ...
      var t = this._map;
      var a = this._rect._latlngs[0];
      var i = new L.LatLng((a[0].lat + a[3].lat) / 2, (a[0].lng + a[3].lng) / 2);
      var n = new L.LatLng((a[1].lat + a[2].lat) / 2, (a[1].lng + a[2].lng) / 2);
      var r = t.layerPointToLatLng(L.PathTransform.pointOnLine(t.latLngToLayerPoint(i), t.latLngToLayerPoint(n), this.options.handleLength));
      this._handleLine = new L.Polyline([n, r], this.options.rotateHandleOptions).addTo(this._handlersGroup);
      var e = this.options.rotateHandleClass;
      this._rotationMarker = new e(r, this.options.handlerOptions).addTo(this._handlersGroup).on("mousedown", this._onRotateStart, this);
      this._rotationOrigin = new L.LatLng((n.lat + i.lat) / 2, (n.lng + i.lng) / 2);
      this._handlers.push(this._rotationMarker);
    }, _getRotationOrigin: function() {
      var t = this._rect._latlngs[0];
      var a = t[0];
      var i = t[2];
      return new L.LatLng((a.lat + i.lat) / 2, (a.lng + i.lng) / 2);
    }, _onRotateStart: function(t) {
      var a = this._map;
      a.dragging.disable();
      this._originMarker = null;
      this._rotationOriginPt = a.latLngToLayerPoint(this._getRotationOrigin());
      ...
      var a = t.layerPoint;
      var i = this._rotationStart;
      var n = this._rotationOriginPt;
      this._angle = Math.atan2(a.y - n.y, a.x - n.x) - Math.atan2(i.y - n.y, i.x - n.x);
      ...
      var a = this._angle;
      this._apply();
      this._path.fire("rotateend", { layer: this._path, rotation: a });
    }, _onScaleStart: function(t) {
      var a = t.target;
      var i = this._map;
      i.dragging.disable();
      this._activeMarker = a;
      this._originMarker = this._handlers[(a.options.index + 2) % 4];
      ...
      var a = this._originMarker._point;
      var i, n;
      if (this.options.uniformScaling) {
        i = a.distanceTo(t.layerPoint) / this._initialDist;
        n = i;
      } else {
        i = (a.x - t.layerPoint.x) / this._initialDistX;
        n = (a.y - t.layerPoint.y) / this._initialDistY;
      }
      this._scale = new L.Point(i, n);
      this._matrix = this._initialMatrix.clone().scale(this._scale, a);
      this._update();
      ...
      this._handlersGroup.eachLayer(function(t2) {
        t2.bringToFront();
      });
      for (var t = 0, a = this._handlers.length; t < a; t++) {
        var i = this._handlers[t];
        i._initialPoint = i._point.clone();
      }
    ...
    }, _createHandler: function(t, a, i) {
      var n = this.options.handleClass;
      var r = new n(t, L.Util.extend({}, this.options.handlerOptions, { className: "leaflet-drag-transform-marker drag-marker--" + i + " drag-marker--" + a, index: i, type: a }));
      ...
      var a = this._rect;
      var i = (t.layer ? t.layer : this._path).dragging._matrix.slice();
      if (!a.dragging) {
        a.dragging = new L.Handler.PathDrag(a);
      }
      a.dragging.enable();
      this._map.addLayer(a);
      a.dragging._transformPoints(i);
      a._updatePath();
      a._project();
      a.dragging.disable();
      this._map.addLayer(this._handlersGroup);
      this._updateHandlers();
      this._path.fire("transformed", { scale: L.point(1, 1), rotation: 0, matrix: L.matrix.apply(void 0, i), translate: L.point(i[4], i[5]), layer: this._path });
    } });
    L.Path.addInitHook(function() {
...
// dep:leaflet-path-transform
var leaflet_path_transform_default = require_L_Path_Transform();
export {
  leaflet_path_transform_default as default
};
/**
 ...
 * @preserve
 */
//# sourceMappingURL=leaflet-path-transform.js.map
