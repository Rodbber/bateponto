...
export default {
  ...
  methods: {
    ...
    centerOfPolygon(arr) {
      ...
      return isNaN(cxTimes6SignedArea / sixSignedArea) ||
        isNaN(cyTimes6SignedArea / sixSignedArea)
        ? null
        : [
    ...
    quadrilatero(coords, nome = null, zoom = null) {
      //console.log(coords)
      let r = L.rectangle(coords, { transform: true, draggable: true }).addTo(
        desenhos
      );
    ...
    editaPonto(row) {
      ...
      const pontos = row.pontos ? JSON.parse(row.pontos) : null;
      if(!pontos){
        this.error('Erro ao pegar dados do ponto');
      }
    ...
    salvarPonto() {
      ...
      if (layer instanceof L.Polygon || layer instanceof L.Rectangle) {
        ...
        let message = "Poligono cadastrado";
        let messageError = "Erro ao cadastrar poligono";
        let url = "/empresa/pontos/poligono/createPonto";
        if (this.id) {
          url = "/empresa/pontos/poligono/update/" + this.id;
          message = "Poligono editado";
          messageError = "Erro ao editar poligono";
        }
        axios
          .post(url, { pontos, nome: this.nome })
          .then((r) => {
    ...
    clearDesenhos() {
      if (desenhos.getLayers().length) {
        if (
          desenhos.getLayers()[0] instanceof L.Rectangle ||
          desenhos.getLayers()[0] instanceof L.Polygon
        ) {
    ...
    getPontos() {
      ...
      //return;
      axios
        .get("/empresa/pontos/poligono/get")
        .then((r) => {
          if (r.data) {
            if (this.data.length) {
    ...
    map.on("click", function (e) {
      if (!testaponto()) {
        ...
        if (desenhos.getLayers().length) {
          ...
          marker.openPopup();
          const markerDentro = `marker dentro<br>${e.latlng}`;
          const markerFora = `marker fora<br>${e.latlng}`;
          //console.log(this.$store.state.empresa)
          axios
            .post("/empresa/pontos/validarPontoDentro/" + getid(), { lat, lng })
            .then((r) => {
              //console.log(r.data)
              if (r.data) {
